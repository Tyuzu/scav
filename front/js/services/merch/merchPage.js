...
export async function displayMerch(contentContainer, merchID, isLoggedIn) {
    ...
    try {
        ...
        const actionRow = createElement("div", { style: "display:flex;gap:8px;align-items:center;margin-top:12px;" }, []);
        ...
        addToCartBtn.addEventListener("click", () => {
            ...
            if (isNaN(qty) || qty < 1 || qty > data.stock) return;
            addToCart({
                itemId: data.merchid,
                quantity: qty,
        ...
        actionRow.append(createElement("label", {}, ["Qty:"]), qtyInput, addToCartBtn);
        ...
        if (data.entity_type && data.entity_id) metaInfo.appendChild(createElement("a", { href:`/${data.entity_type}/${data.entity_id}`, style:"color:#1976D2;text-decoration:none;" }, [`View related ${data.entity_type}`]));
