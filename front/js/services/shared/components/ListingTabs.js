/**
 * Consolidated ListingTabs Component
...
function createCardSection() {
  ...
  const fetchPage = async (category, skip, limit) =>
    apiFetch(
      `/homecards?category=${encodeURIComponent(category)}&skip=${skip}&limit=${limit}`
    );

  const showMessage = (msg) =>
    createElement("p", { class: "status-message" }, [msg]);
