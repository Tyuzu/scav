...
func UploadFile(ctx context.Context, file multipart.File, filePath, userID, postID string) error {
	// Validate filePath to prevent path traversal attacks
	if strings.Contains(filePath, "..") || strings.HasPrefix(filePath, "/") {
		return fmt.Errorf("invalid file path: contains traversal sequences")
	}
	...
	if updateResult.MatchedCount == 0 {
		...
		_, err = db.FilesCollection.InsertOne(ctx, newFile)
		if err != nil {
